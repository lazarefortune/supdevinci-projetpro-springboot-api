...
public class StockServiceImpl implements StockService {
  ...
  public Stock findByProductName(String productName) {
    ...
    if (product == null) {
      throw new ApiRequestException(
          "Product with name : " + product.getName() + " does not exist", HttpStatus.NOT_FOUND);
    }
    Stock stock = stockRepository.findByProductId(product.getId());
    if (stock == null) {
      throw new ApiRequestException(
          "Stock not found for product : " + product.getName(), HttpStatus.NOT_FOUND);
    }
    return stock;
  }
  ...
  public Stock createOne(Stock stock) {
    ...
    if (product == null) {
      throw new ApiRequestException(
          "Product with name " + stock.getProduct().getName() + " does not exist",
          HttpStatus.NOT_FOUND);
    }
    Stock existingStock = stockRepository.findByProductId(product.getId());
    if (existingStock != null) {
      throw new ApiRequestException(
          "Stock already exists for product with name : " + product.getName());
    }
  ...
  public Stock updateOne(Stock stock) {
    ...
    if (existingStock == null) {
      throw new ApiRequestException(
          "Stock does not exist for product id: " + product.getId(), HttpStatus.NOT_FOUND);
    }
